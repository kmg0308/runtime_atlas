...
public enum UpdateReleasePolicy {
    public enum AppBundleValidationError: Error, Equatable, CustomStringConvertible {
        ...
        public var description: String {
            ...
            case .menuBarOnlyApp:
                return "Downloaded app must be a normal windowed app."
            case .backgroundOnlyApp:
                return "Downloaded app must not be background-only."
            case .missingExecutable:
    ...
    public static let runtimeAtlasArchiveName = "RuntimeAtlas.app"
    public static let runtimeAtlasAppName = "Runtime Atlas"
    ...
    public static func isInstallableRuntimeAtlasZipAssetName(_ value: String) -> Bool {
        let name = value.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        if name == "runtimeatlas.zip" {
            return true
        }

        let prefix = "runtimeatlas-"
        let suffix = ".zip"
    ...
    public static func runtimeAtlasZipDownloadName(version: String) -> String {
        let safeVersion = safeFileComponent(normalizedVersion(version))
        guard !safeVersion.isEmpty else { return "RuntimeAtlas.zip" }
        return "RuntimeAtlas-\(safeVersion).zip"
    }

    public static func validateRuntimeAtlasAppBundle(
        at url: URL,
        ...
        guard bundleIdentifier == runtimeAtlasBundleIdentifier,
              executable == runtimeAtlasExecutableName else {
        ...
        guard bundleName == runtimeAtlasAppName else {
        ...
        let executableURL = url
            ...
            .appendingPathComponent(runtimeAtlasExecutableName)
